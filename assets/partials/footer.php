<footer role="contentinfo">
  <section class="logo-info">
    <a class="logo-desktop-bottom-wrapper" href="/lami-fritsch-version-backend/index.html">
      <span class="sr-only">Retour à l'accueil</span>
      <img class="logo-desktop-vertical" src="/lami-fritsch-version-backend/assets/img/svg/logo-vertical.svg" alt="Logo vertical du restaurant L'ami Fritsch">
    </a>
    <div class="horaires-cards">
      <p class="horaires">
        Ouvert de mercredi à vendredi entre 12h et 14h et de jeudi à dimanche entre 18h45 et 22h
      </p>
      <div class="cards">
        <div class="contact-card">
          <address>
            L'ami Fritsch<br>
            9, rue principale, 67350<br>
            Ettendorf<br>
            Tél : <a href="tel:0368038167">03.68.03.81.67</a><br>
            Mail : <a href="mailto:lamifritsch@orange.fr">lamifritsch@orange.fr</a>
          </address>
        </div>
        <div class="navigation-card" role="navigation" aria-label="Liens du pied de page">
          <ul class="gauche">
            <li><a href="/lami-fritsch-version-backend/index.html">Accueil</a></li>
            <li><a href="/lami-fritsch-version-backend/carte-et-menus.html">Carte et menus</a></li>
            <li><a href="/lami-fritsch-version-backend/reservations.html">Réservations</a></li>
          </ul>
          <ul class="droite">
            <li><a href="/lami-fritsch-version-backend/contact.html">Groupe</a></li>
            <li><a href="/lami-fritsch-version-backend/mentions-legales.html" rel="nofollow">Mentions légales</a></li>
            <li><a href="/lami-fritsch-version-backend/protections-donees.html" rel="nofollow">Protections des données</a></li>
          </ul>
        </div>
      </div>
    </div>
    <div class="socials">
      <a class="pastille" target="_blank" href="https://www.instagram.com/lamifritsch/">
        <span class="sr-only">Suivez-nous sur Instagram</span>
        <img src="/lami-fritsch-version-backend/assets/icons/instagram-brands.svg" alt="Logo Instagram">
      </a>
      <a class="pastille" target="_blank" href="https://www.facebook.com/Lamifritsch/">
        <span class="sr-only">Suivez-nous sur Facebook</span>
        <img src="/lami-fritsch-version-backend/assets/icons/facebook-f-brands-solid.svg" alt="Logo Facebook">
      </a>
    </div>
  </section>
</footer>
<a href="./"></a>